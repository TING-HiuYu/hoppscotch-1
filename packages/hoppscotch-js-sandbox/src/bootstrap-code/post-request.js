  ...
  if (!globalThis.__createChaiProxy) {
    globalThis.__createChaiProxy = function (
      ...
      proxy.equal = (expected) =>
        inputs.chaiEqual(expectVal, expected, modifiers, "equal")
      proxy.equals = (expected) =>
        inputs.chaiEqual(expectVal, expected, modifiers, "equals")
      proxy.eq = (expected) =>
        inputs.chaiEqual(expectVal, expected, modifiers, "eq")
      proxy.eql = (expected) => inputs.chaiEql(expectVal, expected, modifiers)

      // Custom Postman methods - delegates to chai-helpers.ts
      proxy.jsonSchema = (schema) => {
        // Debug: ensure chai method exists
      ...
      aMethod.instanceof = function (constructor) {
      ...
      // Add .instanceof as a property of the function
      anMethod.instanceof = function (constructor) {
        const objectType = Object.prototype.toString.call(expectVal)
        let constructorName = "Unknown"
        try {
          if (constructor && typeof constructor.name === "string") {
      ...
      // Contain can be both a method and have properties (for .contain.oneOf)
      ...
      proxy.lengthOf = (length) => {
        ...
        if (expectVal instanceof Set) {
          ...
          typeName = "Set"
          serializedContent = Array.from(expectVal) // Convert Set to array
        } else if (expectVal instanceof Map) {
          actualSize = expectVal.size
          typeName = "Map"
          serializedContent = Array.from(expectVal) // Convert Map to array of [key, value] pairs
        }
        inputs.chaiLengthOf(
      ...
      proxy.length = (length) => {
        ...
        return withModifiers(modifiers + ` length ${length}`)
      }

      proxy.property = (prop, val) => {
        inputs.chaiProperty(expectVal, prop, val, modifiers)
        return withModifiers(modifiers)
      }
      proxy.ownProperty = (prop) => {
        inputs.chaiOwnProperty(expectVal, prop, modifiers)
        return withModifiers(modifiers)
      }
      ...
      proxy.keys = (...keys) => inputs.chaiKeys(expectVal, keys, modifiers)
      proxy.key = (...keys) => inputs.chaiKeys(expectVal, keys, modifiers)
      ...
      proxy.change = (obj, prop) => {
        // Execute function and track property changes
        let initialValue
        let finalValue
        let changed = false
        let delta = 0
        ...
        inputs.chaiChange(prop, modifiers, changed, delta)

        // Return a proxy with .by() method for chaining
        return {
          by: (expectedDelta) => {
            inputs.chaiChangeBy(prop, modifiers, changed, delta, expectedDelta)
          },
        }
      }

      proxy.increase = (obj, prop) => {
        // Execute function and check if value increased
        let initialValue
        let finalValue
        let increased = false
        let delta = 0
        ...
        // Call the assertion (adds result to testStack)
        inputs.chaiIncrease(prop, modifiers, increased, delta)
      ...
      proxy.decrease = (obj, prop) => {
        // Execute function and check if value decreased
        let initialValue
        let finalValue
        let decreased = false
        let delta = 0
        ...
        // Call the assertion (adds result to testStack)
        inputs.chaiDecrease(prop, modifiers, decreased, delta)
      ...
      // PRE-CHECK PATTERN: Check instanceof for built-in types BEFORE serialization
      ...
      Object.defineProperty(proxy, "have", {
        get: () => {
          ...
          haveProxy.ownPropertyDescriptor = (prop, descriptor) => {
            ...
            return inputs.chaiOwnPropertyDescriptor(
              expectVal,
              prop,
              descriptor,
              modifiers + " have",
        ...
        get: () => {
          ...
          ownProxy.property = (prop, val) => {
            if (val !== undefined) {
              ...
              inputs.chaiOwnProperty(expectVal, prop, newModifiers)
            }
          }
          return ownProxy
        },
  ...
  globalThis.pm = {
    environment: {
      ...
      set: (key, value) => {
        ...
        return globalThis.hopp.env.active.set(key, value)
      },
      unset: (key) => {
    ...
    globals: {
      ...
      set: (key, value) => {
        ...
        return globalThis.hopp.env.global.set(key, value)
      },
      unset: (key) => {
        // Remove from tracking when unset
        if (globalThis.__pmGlobalKeys) {
