...
export const createChaiMethods: (
  ...
  return {
    // Equality assertions
    chaiEqual: defineSandboxFn(
      ctx,
      "chaiEqual",
      function (value: any, expected: any, modifiers: any, methodName: any) {
        executeChaiAssertion(
          () => applyModifiers(value, modifiers || " to").equal(expected),
          buildMessage(value, modifiers || " to", methodName || "equal", [
            expected,
          ])
        )
      }
    ...
    chaiTypeOf: defineSandboxFn(
      ...
      (value: any, type: any, modifiers?: any, preCheckedType?: any) => {
        ...
        if (preCheckedType !== undefined) {
          const matches = String(preCheckedType) === String(type)
    ...
    chaiInstanceOf: defineSandboxFn(
      ...
      function (
        ...
        if (isBuiltIn) {
          ...
          // For custom types, we can't reliably detect after serialization
          // This will always fail for custom constructors
          isInstance = false
        }
    ...
    chaiLengthOf: defineSandboxFn(
      ...
      function (
        ...
        if (actualSize !== undefined && typeName) {
          ...
          executeChaiAssertion(
            () => chai.expect(value).to.have.lengthOf(Number(length)),
    ...
    chaiOwnProperty: defineSandboxFn(
      ctx,
      "chaiOwnProperty",
      (value: any, property: any, modifiers?: any) => {
        const mods = modifiers || " to"
        executeChaiAssertion(
          () => {
            const assertion = applyModifiers(value, mods)
    ...
    chaiKeys: defineSandboxFn(
      ...
      (value: any, keys: any, modifiers?: any) => {
        const mods = modifiers || " to"
        executeChaiAssertion(
          () => chai.expect(value).to.have.keys(...keys),
    ...
    chaiMembers: defineSandboxFn(
      ...
      (value: any, members: any, modifiers?: any) => {
        const mods = modifiers || " to"
        executeChaiAssertion(
          () => chai.expect(value).to.have.members(members),
    ...
    chaiIncludeMembers: defineSandboxFn(
      ...
      (value: any, members: any, modifiers?: any) => {
        const mods = modifiers || " to"
        executeChaiAssertion(
          () => chai.expect(value).to.include.members(members),
    ...
    chaiOneOf: defineSandboxFn(
      ...
      (value: any, list: any, modifiers?: any) => {
        ...
        const assertion = isInclude ? "oneOf" : "oneOf"
        executeChaiAssertion(
          () => {
            if (isInclude) {
              chai.expect(value).to.include.oneOf(list)
            } else {
              chai.expect(value).to.be.oneOf(list)
            }
          },
    ...
    chaiThrow: defineSandboxFn(
      ...
      function (
        ...
        if (isNegated) {
          // For negated (.not.throw), should pass if did NOT throw
